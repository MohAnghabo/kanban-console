...
function branchIsProtected(branch: string | null | undefined): boolean {
  return (
    branch === null ||
    branch === undefined ||
    branch === "" ||
  ...
  return ProductArtifactsProvider.of({
    ...
    writeArtifact: (input) =>
      Effect.gen(function* () {
        ...
        const branchDisplay = details.branch ?? "detached HEAD";
        if (branchIsProtected(details.branch)) {
          return {
            repoId: input.repoId,
            path: confined.relativePath,
            status: "blocked",
            message: `Artifact edits are blocked on protected branch ${branchDisplay}.`,
          } satisfies KanbanConsoleArtifactWriteResult;
        }
