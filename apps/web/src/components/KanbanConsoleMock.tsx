...
): ArtifactSaveStatusKey {
  return artifact?.status === "clean" ? "artifactClean" : "artifactBlocked";
}
...
function ArtifactsView({
  ...
  const [saveStatus, setSaveStatus] = useState<ArtifactSaveStatusKey>(
    artifactSaveStatusKey(selectedArtifact),
  );
          ...
          {snapshot.artifacts.map((artifact) => (
            <button
              ...
              onClick={() => {
                ...
                setSaveStatus(artifactSaveStatusKey(artifact));
              }}
            >
        ...
        <div className="rounded-md border border-border bg-card p-3">
          <div className="mb-2 flex gap-2">
            ...
            <Button
              size="xs"
              variant="outline"
              onClick={() => setSaveStatus(artifactSaveStatusKey(selectedArtifact))}
            >
              <CheckCircle2Icon />
          ...
          <DetailRow
            label={messages.artifactPathLabel}
            value={selectedArtifact?.path ?? "docs/product"}
          />
          <DetailRow label={messages.artifactGuardLabel} value={messages[saveStatus]} />
